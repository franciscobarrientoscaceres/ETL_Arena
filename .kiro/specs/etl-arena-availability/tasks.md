...
- [x] **Checkpoint E — code-reviewer**: transacción y bloqueo de `publicar_mes`, protecciones, permisos de `etl_writer`, idempotencia, que `parity_failed` nunca publique, paridad intacta (golden + COM). *(hecho 2026-09-26 por Claude. Verificado: `sp_getapplock` corre dentro de la transacción (`@@TRANCOUNT = 1`) y una segunda publicación espera y recibe −1 hasta el commit de la primera (probado con dos conexiones); rollback total ante falla inyectada; protecciones D-21/D-23 y `parity_failed`/`golden` sin publicar (tests unitarios e integración); `etl_writer` con `DELETE/UPDATE` solo en tablas de estado; recarga idéntica en Azure TEST/QA sin cambios; golden y COM en verde (5 tests COM, incl. nuevo `tests/com/test_exclusion_com.py`). Corregido: E-1 con varios meses el libro base se promovía después de cada mes aunque las macros del mes siguiente lo modificaban; si ese mes no cuadraba, el puntero quedaba con un sha256 viejo y el lunes siguiente fallaba: ahora se promueve una vez al final (test `test_varios_meses_promueven_el_libro_una_vez_al_final`); E-2 `SET NOCOUNT OFF` se restablece también si la publicación falla (la conexión vuelve al pool). Observaciones: si una recarga de varios meses se detiene en un mes, los anteriores ya quedaron publicados y el libro base no avanza (repetir el mismo comando retoma); una revisión de detención se liga por (PCS, FechaInicio), así que un evento de las 23:45 del último día que aparece en dos meses muestra la misma revisión en ambos.)*
