...
- [ ] 13. Pipeline orquestador
  ...
  - [ ] 13.4 Implementar golden tests en `tests/golden/`
    ...
    - Crear `tests/golden/test_gt_september_2026.py`: test especifico para septiembre 2026 (nota: en el golden `fault_code` = codigo corto como `"F55"` y `fault_description` = descripcion larga como `"F55 EXTERNAL FAULT/OVGR"`) (periodo parcial 01-21 sep) usando `tests/golden/data/golden_2026_09_september.json` — verifica `BloquesMuestreo=1975`, `BloquesRacksIndisponibles=104134.2920000001`, `DisponibilidadPeriodo=0.9819924099052362`, `DisponibilidadAnualAcumulada=0.9989850173961998`, los 21 dias con datos reales (daily_unavailable > 0) y los 9 dias 22-30 con `BloquesRacksIndisponiblesDiarios=0` pero `Disponibilidad > 0` (el denominador crece aunque el acumulado quede fijo, igual que el Excel), y los primeros 10 eventos del PCS 1
    ...
    - `tests/golden/test_golden_integrity.py` YA EXISTE: invariantes sobre los 3 JSON (`sum(daily)~=C14`, orientacion de campos, `rh~=12*dur*avg`, `C16~=1-C14/(Racks*C12)`). Correrlo en CI junto al runner.
    - **GT-5 / Annual_AVA:** las filas historicas de jul/ago **no** son gate de paridad (divergen del C14 de la corrida actual). Ver design.md GT-5. No crear tests que exijan 350972/305182 contra la corrida Python.
    - **Discrepancia sep 1975 vs 1977: RESUELTA (2026-09-24).** `C12=1975` es correcto (filas RawData 01-21 con gap DST 2026-09-06). `sampling_blocks=1977` de Annual_AVA es formula de calendario; no usar como esperado de BloquesMuestreo. Detalle en `golden_index.json` → `discrepancies`.
    - El script `tests/golden/extract_golden.py` ya existe, valida invariantes y falla si `sum(daily)!=C14` o la orientacion de campos es incorrecta. Uso: `python tests/golden/extract_golden.py --excel <ruta> --month <N> --year 2026 --label <nombre>`
    - Estados actuales del indice (2026-09-24): los 3 meses estan `pending` hasta re-corrida de macros + re-extract limpio (julio: posible cap 295 eventos; agosto: daily hand-fix; sept: periodo parcial). Tras re-extract verificado, marcar `verified`.
