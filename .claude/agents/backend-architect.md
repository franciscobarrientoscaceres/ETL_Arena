...
description: Arquitecto backend del ETL Arena: entorno Python, persistencia SQL Server (SQLAlchemy/pyodbc), pipeline y CLI, automatización COM de Excel, adquisición SCADA, orquestador run_lunes y notificación. Usar para las tareas de tasks.md asignadas a backend-architect.
---
...
- SQL Server append-only por `IdCorrida`.
...
Notas específicas: SQL Server 2022 con ODBC Driver 18 y `fast_executemany`; una transacción por corrida. Para COM usa `pywin32` con `DispatchEx`, instancia visible, cierra solo la instancia propia y aplica watchdog de timeout.
