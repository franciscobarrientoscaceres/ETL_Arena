...
| ID | Tema | Estado |
|---|---|---|
| D-11 | El KPI oficial de agosto en `Annual_AVA` (305.182,968) no se reproduce ni excusando (256.223,356) ni sin excusar (527.396,044) con el libro actual. Hipótesis: `PlantActivity!D` cambió después del cierre de agosto. Con la regla nueva (F-37) agosto da 264.731,768: tampoco coincide. Se importa como `excel_manual` (R10.4); investigar con el libro de cierre de agosto | **Postergada** (2026-09-24): prioridad septiembre; Francisco solicitará el libro de cierre de agosto |
| D-12 | Quién actualiza `PlantActivity` (columnas B/C/D) para las filas nuevas | **Resuelta, alcance reducido por F-37** (2026-09-24): PlantActivity ya solo aporta la columna C ("Only Operational Time?", sin uso hoy); la exclusión la da `Exclusion_Matrix` (D-17). Original: la entrega **Alex** (ingeniero eléctrico a cargo) **una vez al mes**. Consecuencias: (1) las corridas semanales son **preliminares** respecto a excusables (`ExcusablesPendientes = 1`, avisado en la notificación); (2) el `cierre_mensual` oficial solo se ejecuta después de cargar la PlantActivity del mes (etapa `load-plant-activity`), que deja registro de cada celda cambiada en `correccion_dato`. Formato del archivo de Alex: pendiente de muestra (tarea 0.8) |
...
| D-16 | ¿Los eventos de falla (`ListOfFaults`, L14) también aplican `Exclusion_Matrix`? | **Aplicada por defecto** (técnico, 2026-09-24): sí, misma regla que el KPI para que `C14 = 4·L10`. La macro de agosto no lo hace (sigue con `PlantActivity!D`). Confirmar con negocio | Pendiente confirmación |
| D-17 | Quién mantiene la `Exclusion_Matrix` y cuándo se entrega (reemplaza en la práctica a la entrega mensual de PlantActivity de D-12) | Hoy la cargó F. Barrientos, solo agosto. Mientras no esté cargada la del mes, el KPI semanal es preliminar (`ExcusablesPendientes`) | Pendiente (negocio / Alex) |
| D-18 | Marca aislada en `Exclusion_Matrix` fila 2 (2026-04-08 00:15): PCS61 = 1 y "Excused Event" = 1 (F-39) | Se respeta tal cual; confirmar si es un residuo | Pendiente (F. Barrientos) |
